...
public class CrushingWeightCurse extends Spell {

    public CrushingWeightCurse(Witchcraft plugin) {
        super(plugin, "crushing_weight_curse", "Curse of Crushing Weight",
                SpellCategory.CURSE,
        ...
        switch (result) {
            case SUCCESS -> {
                ...
                for (Player enemy : enemies) {
                    enemy.addPotionEffect(new PotionEffect(PotionEffectType.SLOWNESS, 600, 3));
                    enemy.addPotionEffect(new PotionEffect(PotionEffectType.MINING_FATIGUE, 600, 3));
                    enemy.addPotionEffect(new PotionEffect(PotionEffectType.WEAKNESS, 400, 1));
                    enemy.sendMessage("\u00A75\u00A7cCrushing weight bears down on you!");
                }
                caster.sendMessage("\u00A75\u00A7lCrushing weight burdens " + enemies.size() + " enemies!");
                playEffects(caster.getLocation());
            }
            case FAILURE -> caster.sendMessage(plugin.getConfigManager().getMessage("ritual.ritual-failure"));
            case BACKFIRE -> {
                caster.addPotionEffect(new PotionEffect(PotionEffectType.SLOWNESS, 300, 2));
    ...
    private void playEffects(Location center) {
        ...
        world.spawnParticle(org.bukkit.Particle.CRIT, center.clone().add(0, 1, 0), 50, 2, 1, 2);
        world.playSound(center, org.bukkit.Sound.BLOCK_ANVIL_LAND, 1.0f, 0.5f);
    }
}
