...
public class BrittleEarthCurse extends Spell {

    public BrittleEarthCurse(Witchcraft plugin) {
        super(plugin, "brittle_earth_curse", "Curse of Brittle Earth",
                SpellCategory.CURSE,
        ...
        switch (result) {
            case SUCCESS -> {
                ...
                for (Player enemy : enemies) {
                    ...
                    enemy.addPotionEffect(new PotionEffect(PotionEffectType.SLOWNESS, 600, 2));
                    enemy.addPotionEffect(new PotionEffect(PotionEffectType.WEAKNESS, 600, 1));
                    enemy.sendMessage("\u00A75\u00A7cThe earth crumbles beneath your feet!");
                }
                caster.sendMessage("\u00A75\u00A7lBrittle earth shatters " + enemies.size() + " enemies!");
                playEffects(caster.getLocation());
            }
            case FAILURE -> caster.sendMessage(plugin.getConfigManager().getMessage("ritual.ritual-failure"));
            case BACKFIRE -> {
                caster.addPotionEffect(new PotionEffect(PotionEffectType.MINING_FATIGUE, 300, 1));
    ...
    private void playEffects(Location center) {
        ...
        world.spawnParticle(org.bukkit.Particle.BLOCK, center.clone().add(0, 1, 0), 60, 2, 1, 2,
                org.bukkit.Material.GRAVEL.createBlockData());
        world.playSound(center, org.bukkit.Sound.BLOCK_GRAVEL_BREAK, 1.0f, 0.5f);
    }
}
